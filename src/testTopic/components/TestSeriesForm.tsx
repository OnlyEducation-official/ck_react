...
const TestSeriesForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await api<ApiSuccessResponse<TestSeriesSchemaType> | ApiErrorResponse>(`topics/${qid}`);
  ...
  const onSubmit = async (data: TestSeriesSchemaType) => {
    try {
      ...
      const url = isEdit ? `topics/${qid}` : `topics`;

      const res = await api<ApiSuccessResponse<TestSeriesSchemaType> | ApiErrorResponse>(
        url,
        {
          headers: {
