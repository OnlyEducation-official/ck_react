...
type ChapterSchemaType = z.infer<typeof ChapterSchema>;
...
const ChapterForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await api<ApiSuccessResponse<ChapterSchemaType> | ApiErrorResponse>(
        `chapters/${qid}`
      );
  ...
  const onSubmit = async (data: ChapterSchemaType) => {
    try {
      ...
      const url = isEdit ? `chapters/${qid}` : `chapters`;

      const res = await api<ApiSuccessResponse<ChapterSchemaType> | ApiErrorResponse>(url, {
        headers: {
